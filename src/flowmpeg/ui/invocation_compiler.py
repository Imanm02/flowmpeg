...
def _field_arguments(field: UiField, value: object) -> tuple[str, ...]:
    ...
    if field.kind is FieldKind.CHOICE and (
        not isinstance(value, str) or value not in field.choices
    ):
        raise UiValidationError(
            UiValidationIssue(
                code="invalid-choice",
    ...
    if field.kind is FieldKind.TEXT and (not isinstance(value, str) or not value):
        raise _invalid_type(field)
